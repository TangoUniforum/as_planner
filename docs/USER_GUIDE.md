...
| `max_transfers_per_week` | weekly HANDLING BUDGET (transfer moves/week): once a week's moves reach it, the deferrable quality passes (even-out, balancer, variable-quantity, remnant sweep) wait for a calmer week; essential moves (6N fills, arrival make-room, plan-diff) are never blocked — overruns show on the handling gate (WARN >12 / FAIL >15). 0 = off | 15 |
